...
class GroupsService extends BaseService {
  ...
  async beforeCreate(data, options) {
    ...
    // path. The normalization rewrites `stix.aliases`, which is correct for
    // user-driven flows but deviates the persisted group from the bundle
    // source-of-truth. `data.stix` is frozen during import-mode hooks
    // (app/lib/import-safety.js), so a missing gate here would throw a
    // TypeError at the assignment in `_normalizeAliases`.
